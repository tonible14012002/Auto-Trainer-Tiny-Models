import { Progress } from "@/components/ui/progress";
import { cn } from "@/lib/utils";
import type { TextClassificationResult } from "@/schema/model";

interface TextClassificationResultPanelProps {
  result?: TextClassificationResult;
  isPending: boolean;
}

export const TextClassificationResultPanel = (
  props: TextClassificationResultPanelProps
) => {
  const { result, isPending } = props;

  return (
    <div className="rounded-lg overflow-hidden">
      {isPending && (
        <Progress indeterminate className="h-[2px] rounded-b-none" />
      )}
      <div
        className={cn(
          "p-4 break-words flex flex-col gap-1 text-sm min-h-[100px]",
          {
            "animate-pulse": isPending,
          }
        )}
      >
        {result &&
          result.data.map((item, index) => (
            <div key={index}>
              <p className="font-medium">{item.label}</p>
              <p className="text-muted-foreground">{item.score}</p>
            </div>
          ))}
      </div>
    </div>
  );
};
